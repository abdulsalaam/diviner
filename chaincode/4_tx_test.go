...
func initTx(t *testing.T) {
	if txCreator == nil {
		...
		txCreator, _ = pbm.NewMember(key, balance)
	}

	if _, existed, err := ccc.GetStateAndCheck(stub, txCreator.Id); err != nil {
		t.Fatal(err)
	} else if !existed {
		b0, _ := pbm.Marshal(txCreator)
		ccc.MockInvoke(stub, fcnCreate, ccMember, b0)
	}

	if txEvent == nil {
		txEvent, _ = pbl.NewEvent(txCreator.Id, title, outcomes[0], outcomes[1])
	}

	if _, existed, err := ccc.GetStateAndCheck(stub, txEvent.Id); err != nil {
		t.Fatal(err)
	} else if !existed {
		b0, _ := pbl.MarshalEvent(txEvent)
...
func TestMarkets(t *testing.T) {
	initTx(t)
	var market1 *pbl.Market
	var market2 *pbl.Market

	num, _ := cast.ToBytes(100.0)
	resp := ccc.MockInvoke(stub, fcnCreate, ccMarket, []byte(txCreator.Id), []byte(txEvent.Id), num, []byte{1})
	if !ccc.OK(&resp) {
		t.Fatal(resp.Message)
	}

	market1, _ = pbl.UnmarshalMarket(resp.Payload)

	resp = ccc.MockInvoke(stub, fcnCreate, ccMarket, []byte(txCreator.Id), []byte(txEvent.Id), num, []byte{0})
	if !ccc.OK(&resp) {
		t.Fatal(resp.Message)
	}

	market2, _ = pbl.UnmarshalMarket(resp.Payload)

	resp = ccc.MockInvokeWithString(stub, string(fcnMarkets), string(ccTx), "tx", "a", "b")
	if ccc.OK(&resp) {
		t.Fatal("can not invoke with wrong data")
	}
	//fmt.Println(resp)

	resp = ccc.MockInvokeWithString(stub, string(fcnMarkets), string(ccTx), txEvent.Id)
	if !ccc.OK(&resp) {
...
func TestSimulator(t *testing.T) {
	...
	for _, x := range members {
		b0, _ := pbm.Marshal(x)
		if resp := ccc.MockInvoke(stub, fcnCreate, ccMember, b0); !ccc.OK(&resp) {
			t.Fatal(resp.Message)
		}
	}

	b0, _ := pbm.Marshal(mem0)
	if resp := ccc.MockInvoke(stub, fcnCreate, ccMember, b0); !ccc.OK(&resp) {
		t.Fatal(resp.Message)
	}

	event, _ := pbl.NewEvent(mem0.Id, "GO", "Yes", "No")
	e0, _ := pbl.MarshalEvent(event)

	if resp := ccc.MockInvoke(stub, fcnCreate, ccEvent, e0); !ccc.OK(&resp) {
		t.Fatal(resp.Message)
	}

	var market *pbl.Market
	num, _ := cast.ToBytes(100.0)
	if resp := ccc.MockInvoke(stub, fcnCreate, ccMarket, []byte(mem0.Id), []byte(event.Id), num, []byte{0}); !ccc.OK(&resp) {
		t.Fatal(resp.Message)
	} else {
		market, _ = pbl.UnmarshalMarket(resp.Payload)
	}
	...
	mem0, _, _ = ccu.GetMemberAndCheck(stub, mem0.Id)
	...
	resp := ccc.MockInvokeWithString(stub, string(fcnAssets), string(ccTx), event.Id)
	if !ccc.OK(&resp) {
	...
	resp = ccc.MockInvokeWithString(stub, string(fcnAssets), string(ccTx), evt, mkt)
	if !ccc.OK(&resp) {
		t.Fatalf("get assets by event (%s) failed: %v", event.Id, resp.Message)
	}
	...
	resp = ccc.MockInvokeWithString(stub, string(fcnApprove), string(ccTx), event.Id, result)
	if !ccc.OK(&resp) {
	...
	// test market failed
	resp = ccc.MockInvokeWithString(stub, string(fcnSettle), string(ccTx), market.Id, result)
	if ccc.OK(&resp) {
		t.Fatalf("can not settle a settled market")
	}

	// test wrong with mem5
	resp = ccc.MockInvokeWithString(stub, string(fcnBuy), string(ccTx), mem5.Id, yes, "100")
	if ccc.OK(&resp) {
		t.Fatalf("can not buy without enough balance")
	}

	resp = ccc.MockInvokeWithString(stub, string(fcnSell), string(ccTx), mem5.Id, yes, "100")
	if ccc.OK(&resp) {
...
func TestTxWrongData(t *testing.T) {
	...
	shareId = pbl.ShareID(market1.Id, txEvent.Outcomes[0].Id)
...
*/
func checkTx(t *testing.T, cmd, user, share string, volume float64) float64 {
	num, _ := cast.ToBytes(volume)
	resp := ccc.MockInvoke(stub, []byte(cmd), ccTx, []byte(user), []byte(share), num)
	if !ccc.OK(&resp) {
...
func myCheck(t *testing.T, mem *pbm.Member, market *pbl.Market, cmd, share string, volume float64) (*pbm.Member, *pbl.Market, float64) {
	price := checkTx(t, cmd, mem.Id, share, volume)
