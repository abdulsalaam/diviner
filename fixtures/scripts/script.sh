...
# chaincode upgrade
#peer chaincode install -n lmsr -v 2.0 -p diviner/chaincode
#peer chaincode upgrade -o orderer.diviner.info:7050 --tls $CORE_PEER_TLS_ENABLED --cafile /opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/diviner.info/orderers/orderer.diviner.info/msp/tlscacerts/tlsca.diviner.info-cert.pem -C divinerchannel -n lmsr -v 2.0 -c '{"Args":[]}' -P "OR ('DivinerMSP.member')"


#peer chaincode invoke -o orderer.diviner.info:7050 --tls $CORE_PEER_TLS_ENABLED --cafile /opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/diviner.info/orderers/orderer.diviner.info/msp/tlscacerts/tlsca.diviner.info-cert.pem -C divinerchannel -n test2 -c '{"Args":["update", "aa", "ccccccc"]}' -P "OR ('DivinerMSP.member')"
