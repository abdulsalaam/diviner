...
func TestQueryMember(t *testing.T) {
	resp := ccc.MockInvoke(memStub, query, []byte("abc"))
	if ccc.OK(&resp) {
...
func TestCreateMember(t *testing.T) {
	...
	resp := ccc.MockInvoke(memStub, create, b1)
	if !ccc.OK(&resp) {
		t.Fatalf("create membe failed: %v\n", resp.Message)
	}

	resp = ccc.MockInvoke(memStub, create, b2)
	if !ccc.OK(&resp) {
		t.Fatalf("create membe failed: %v\n", resp.Message)
	}

	resp = ccc.MockInvoke(memStub, query, []byte(m1.Id))
...
func TestUpdateMember(t *testing.T) {
	b3, _ := pbm.Marshal(m3)
	resp := ccc.MockInvoke(memStub, update, b3)
	if ccc.OK(&resp) {
	...
	m1.Balance = 1000.0
	b1, _ := pbm.Marshal(m1)
	resp = ccc.MockInvoke(memStub, update, b1)
	if !ccc.OK(&resp) {
